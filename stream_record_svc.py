...
#parser.add_argument('-r', action='store', dest='recordresolution', type=str, default='640x480', help='set the record resolution. Default: 640x480')
#args = parser.parse_args()
#splittedrecordresolution = args.recordresolution.split("x")
...
with open('/usr/local/bin/webstream.conf') as f:
        ...
        for i in range(0, len(lines)):
                ...
                if(lines[i].find("Recordresolution") != -1):
                        Recordresolution = lines[i].split('Recordresolution = "')
                        Recordresolution = Recordresolution[1]
                        Recordresolution = Recordresolution.strip('\"\n')
                        Recordresolution = Recordresolution.split('x')
                        RecordresolutionX= Recordresolution[0]
                        RecordresolutionY= Recordresolution[1]
                if(lines[i].find("VideoFileDestination") != -1):
                        VideoFileDestination = lines[i].split('VideoFileDestination = "')
                        VideoFileDestination = VideoFileDestination[1]
                        VideoFileDestination = VideoFileDestination.strip('\"\n')
                        if VideoFileDestination[-1] != '/':
                                VideoFileDestination = VideoFileDestination + '/'
        f.close()
...
with picamera.PiCamera(resolution=Streamresolution, framerate=int(Framerate)) as camera:
    ...
    camera.start_recording(VideoFileDestination + timestr + '.h264', splitter_port=2, resize=(int(RecordresolutionX), int(RecordresolutionY))) #####
    camera.start_recording(output, format='mjpeg')
#    camera.start_recording('vidcap.h264', splitter_port=2, resize=(1600, 1200)) #####
    try:
        address = ('', 8000)
